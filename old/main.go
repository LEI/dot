...
func main() {
	...
	if ConfigFile != "" {
		...
		ConfigFile = filepath.Join(Config.Source, filepath.Base(ConfigFile))
	} else {
...
func handleConfig(path string) error {
	// /etc/dotrc

	err := readConfig(&Config, path)
	fmt.Printf("READCONF %s === %+v\n", path, Config)
	if err != nil && os.IsExist(err) {
		...
		pkg := &Package{}
		err = readConfig(&pkg, path)
		fmt.Println("READPKG", err, path, "->", pkg)
		if err != nil {
...
func readConfig(v interface{}, path string) error {
	var e error
	var paths []string
	for _, ext := range ConfigExts {
		...
		if err != nil {
			...
			// if ext == ".json" {
			// 	fi, err := ioutil.ReadFile(path+ext)
			// 	if err != nil {
			// 		return err
			// 	}
			// 	return json.Unmarshal([]byte(string(fi)), &v)
			// }
	...
	if len(paths) > 0 {
		...
		return fmt.Errorf("%v: configuration not found", paths)
	}
	return nil
}
...
func handlePackage(name string, pkg Package) error {
	...
	err := readConfig(&pkg, pkgConfigFile)
	if err != nil && os.IsExist(err) {
		return err
	}
